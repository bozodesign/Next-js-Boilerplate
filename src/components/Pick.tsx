...
export default function Pick({ pickData }: any) {
  ...
  const itemsToShow = showAll ? pickData.slice(1, 9) : pickData.slice(1, 9);
        ...
        {itemsToShow?.map((item: any, index: number) => (
          <div key={item?.title+index} className='flex flex-col rounded-lg m-1 p-3 col-span-1 bg-white drop-shadow-lg'>
            ...
            <div className='flex flex-row gap-2 mt-1'>
                <div className='w-1/4 relative h-12 border rounded-lg overflow-hidden aspect-square'>
                  {item?.thumbnail_url ? 
                    <Image
                ...
                <div className='w-3/4 px-2 py-1 flex flex-row justify-between items-center bg-black/5 rounded-lg'>
